...
\textbf{C2 --- The instruction points at nothing.} The decoder input reads
\begin{center}
$[\text{soft tokens}]\,[\text{``Convert the following MATLAB code\ldots''}]\,[\text{target}]$:
\end{center}
the model is told to convert code that \emph{follows}, and nothing follows. The instruction, kept verbatim from the text variant, references an object absent from the sequence, further rewarding the unconditional prior.
